...
contract DigiArtPlat{
    ...
    function getPrice(uint id) external view returns(uint){
        return arts[id].currentPrice;
    }

    function getRent(uint id) external view returns(uint){
        return arts[id].currentRent;
    }

    function getBid(uint id) external view returns(uint){
        return arts[id].currentBid;
    }
    ...
    function rent(uint ID/*, uint start, uint duration, string destination*/)public notOwner(arts[ID]) payable returns(uint){
        ...
        arts[ID].rentStart[msg.sender] = now;
        arts[ID].rentDuration[msg.sender] = 0
